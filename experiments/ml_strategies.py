...
def run_portfolio_strategy(
    ...
    # 1️⃣ Chargement et calcul des volatilités
    for sym in symbols:
    ...
    # 2️⃣ Alignement retours et volatilités
    rets = pd.DataFrame({s: dfs[s]["ret"] for s in symbols})
    vol = pd.DataFrame({s: dfs[s]["vol_ewma"] for s in symbols})  # tu peux changer vol_ewma → vol_rolling, vol_parkinson...
    ...
    if strategy == "equal_weight":
        ...
        # Ajouter les colonnes de volatilité dans le dataset ML
        rets_ml = rets.copy()
        for sym in symbols:
            rets_ml[f"{sym}_vol"] = vol[sym]
        
        # Dataset stress
        ds_stress, cutoff = build_stress_dataset(rets_ml)
        ...
        if strategy == "ml_stress":
            ...
            ds_dir, _ = build_direction_dataset(rets_ml)
